...
@Client.on_message(
    command(["userbotjoin", f"userbotjoin@{BOT_USERNAME}", "ادخل"]) & other_filters
)
...
@Client.on_message(
    command(["userbotleave", f"userbotleave@{BOT_USERNAME}", "غادر"]) & other_filters
)
@authorized_users_only
...
@Client.on_message(command(["leaveall", f"leaveall@{BOT_USERNAME}", "اخرج"]))
@sudo_users_only
